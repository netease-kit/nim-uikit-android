...
public class SuspensionDecoration extends RecyclerView.ItemDecoration {
    ...
    public SuspensionDecoration(Context context, List<? extends ISuspension> datas) {
        ...
        indexDecorationHeight = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 6f, context.getResources().getDisplayMetrics());
        mTitleFontSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 14f, context.getResources().getDisplayMetrics());
        bottomLineHeight = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 1f, context.getResources().getDisplayMetrics());
    ...
    public void onDraw(@NonNull Canvas c, @NonNull RecyclerView parent, @NonNull RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (mData == null || mData.isEmpty() || position > mData.size() - 1 || position < 0 || !mData.get(position).isShowDivision()) {
                continue;
            }
            ...
            if (position == 0) {//if 0 must have title
                ...
                if (null != mData.get(position).getTag() && !mData.get(position).getTag().equals(mData.get(position - 1).getTag())) {
    ...
    private void drawTitleArea(Canvas c, int left, int right, View child, RecyclerView.LayoutParams params, int position) {//最先调用，绘制在最下层
        if (isFirstTagDraw) {
            ...
            c.drawRect(left, child.getTop() - params.topMargin - mTitleHeight - indexDecorationHeight,
                    right, child.getTop() - params.topMargin - mTitleHeight, mPaint);
        }
        ...
        c.drawRect(left + paddingLeft, child.getTop() - params.topMargin - bottomLineHeight, right, child.getTop() - params.topMargin, mPaint);

        mPaint.setColor(colorTitleFont);
        mPaint.getTextBounds(mData.get(position).getTag(), 0, mData.get(position).getTag().length(), mBounds);
        c.drawText(mData.get(position).getTag(), child.getPaddingLeft() + paddingLeft, child.getTop() - params.topMargin - ((float) mTitleHeight / 2 - (float) mBounds.height() / 2), mPaint);
    }

    @Override
    public void onDrawOver(@NonNull Canvas c, final RecyclerView parent, @NonNull RecyclerView.State state) {
        if (parent.getLayoutManager() == null) {
            return;
        }
        int pos = ((LinearLayoutManager) (parent.getLayoutManager())).findFirstVisibleItemPosition();
        pos -= getHeaderViewCount();
        if (mData == null || mData.isEmpty() || pos > mData.size() - 1 || pos < 0 || !mData.get(pos).isSuspension()) {
            return;
        }

        String tag = mData.get(pos).getTag();
        RecyclerView.ViewHolder viewHolder = parent.findViewHolderForLayoutPosition(pos + getHeaderViewCount());
        if (viewHolder == null) {
            return;
        }
        ...
        c.drawText(tag, child.getPaddingLeft() + 40,
                parent.getPaddingTop() + mTitleHeight - ((float) mTitleHeight / 2 - (float) mBounds.height() / 2),
                mPaint);
        if (flag)
            c.restore();
    }
        ...
        if (position >= 0) {
            ...
            if (titleCategoryInterface.isShowDivision()) {
                if (position == 0) {
                    ...
                    if (null != titleCategoryInterface.getTag() && !titleCategoryInterface.getTag().equals(mData.get(position - 1).getTag())) {
                        outRect.set(0, mTitleHeight + (isFirstTagOffset ? indexDecorationHeight : 0), 0, 0);
                        isFirstTagOffset = false;
                    }
                }
