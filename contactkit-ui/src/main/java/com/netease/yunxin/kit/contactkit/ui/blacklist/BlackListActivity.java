...
public class BlackListActivity extends BaseListActivity {
    ...
    protected void initView() {
        ...
        binding.title.setTitle(R.string.black_list)
        ...
        binding.contactListView.setViewHolderFactory(new ContactViewHolderFactory() {
            ...
            protected BaseContactViewHolder getCustomViewHolder(ViewGroup view, int viewType) {
                if (viewType == IViewTypeConstant.CONTACT_BLACK_LIST) {
                    ...
                    viewHolder.setRelieveListener(data -> viewModel.removeBlackOp(data.data.getAccount(), new FetchCallback<Void>() {

                        @Override
                        public void onException(@Nullable Throwable exception) {
                            Toast.makeText(BlackListActivity.this, getText(R.string.remove_black_fail), Toast.LENGTH_SHORT).show();
                        }
    ...
    private void registerResult() {
        blackListLauncher = registerForActivityResult(new ActivityResultContracts.StartActivityForResult(), result -> {
            if (result.getResultCode() == RESULT_OK && result.getData() != null) {
                ArrayList<String> friends = result.getData().getStringArrayListExtra(REQUEST_CONTACT_SELECTOR_KEY);
                if (friends != null && friends.size() > 0) {
                    for (String account : friends) {
                        viewModel.addBlackOp(account, new FetchCallbackImpl<Void>(account) {
                            @Override
                            public void onFailed(int code) {
                                String content = String.format(getResources().getString(R.string.add_black_error), account);
                                Toast.makeText(BlackListActivity.this, content, Toast.LENGTH_SHORT).show();
