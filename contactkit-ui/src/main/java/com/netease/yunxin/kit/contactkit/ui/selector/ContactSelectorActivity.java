...
public class ContactSelectorActivity extends BaseActivity {
        ...
        actions.addSelectorListener(IViewTypeConstant.CONTACT_FRIEND, (selector, data) -> {
            if (selector) {
                if (selectedListAdapter.getItemCount() >= maxSelectCount) {
                    Toast.makeText(this, getString(R.string.contact_selector_max_count, String.valueOf(maxSelectCount)), Toast.LENGTH_LONG).show();
    ...
    protected void initData() {
        ...
        maxSelectCount = getIntent().getIntExtra(RouterConstant.KEY_CONTACT_SELECTOR_MAX_COUNT, DEFAULT_MAX_SELECT_COUNT);
        enableReturnName = getIntent().getBooleanExtra(RouterConstant.KEY_REQUEST_SELECTOR_NAME_ENABLE, false);
        viewModel.getContactLiveData().observe(this,
                contactBeansResult -> {
                    if (contactBeansResult.getLoadStatus() == LoadStatus.Success) {
                        binding.contactListView.onFriendDataSourceChanged(filterUser(contactBeansResult.getData()));
                    }
                });
