...
public class SearchViewModel extends BaseViewModel {
    ...
    public void query(String text) {
        ...
        if (!TextUtils.isEmpty(text)) {
            SearchRepo.INSTANCE.searchFriend(text, new FetchCallback<List<FriendSearchInfo>>() {
                @Override
                public void onSuccess(@Nullable List<FriendSearchInfo> param) {
                    if (param != null && param.size() > 0) {
                        ...
                        for (int index = 0; index < param.size(); index++) {
                            ...
                            XLog.d(TAG,"searchFriend:onSuccess","friend:"+param.get(index).getFriendInfo().getName());
                        }
            ...
            SearchRepo.INSTANCE.searchGroup(text, new FetchCallback<List<TeamSearchInfo>>() {
                @Override
                public void onSuccess(@Nullable List<TeamSearchInfo> param) {
                    if (param != null && param.size() > 0) {
                        ...
                        for (int index = 0; index < param.size(); index++) {
                            ...
                            XLog.d(TAG,"searchTeam:onSuccess","team:"+param.get(index).getTeam().getName());
                        }
            ...
            SearchRepo.INSTANCE.searchTeam(text, new FetchCallback<List<TeamSearchInfo>>() {
                @Override
                public void onSuccess(@Nullable List<TeamSearchInfo> param) {
                    if (param != null && param.size() > 0) {
                        List<BaseBean> friendResult = new ArrayList<>();
