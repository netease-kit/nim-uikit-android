...
public class TeamViewHolder extends BaseViewHolder<TeamBean> {
    ...
    public void onBindData(TeamBean data, int position) {
        if (data != null){
            ...
            viewBinding.cavUserIcon.setData(searchInfo.getTeam().getIcon(),searchInfo.getTeam().getName(), AvatarColor.avatarColor(searchInfo.getTeam().getId()));
            viewBinding.tvNickName.setText(getSelectSpanText(searchInfo.getTeam().getName(), searchInfo.getHitInfo()));
