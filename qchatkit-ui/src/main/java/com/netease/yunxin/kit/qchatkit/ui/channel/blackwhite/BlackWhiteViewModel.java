...
/**
 * black name list and white name list view model
 * to fetch member list and add member to member list
 */
public class BlackWhiteViewModel extends BaseViewModel {
     ...
     * fetch member list
     */
    private void fetchMemberData(long serverId, long channelId, QChatChannelModeEnum type, long offset) {
        ALog.d(TAG, "fetchMemberData");
        QChatChannelRepo.fetchChannelBlackWhiteMembers(serverId, channelId, offset, type, QChatConstant.MEMBER_PAGE_SIZE, new FetchCallback<List<QChatServerMemberInfo>>() {
            @Override
     ...
     */
    public void deleteMember(long serverId, long channelId, int channelType, String accId, int position) {
        ArrayList<String> accIdList = new ArrayList<>();
        accIdList.add(accId);
        QChatChannelRepo.removeChannelBlackWhiteMembers(serverId, channelId, channelType, accIdList, new FetchCallback<Void>() {
            @Override
            ...
            public void onException(@Nullable Throwable exception) {
                String errorMsg = exception != null ? exception.getMessage() : "";
                ALog.d(TAG, "fetchMemberData", "onException" + errorMsg);
                addResult.setError(QChatConstant.ERROR_CODE_CHANNEL_MEMBER_ADD, R.string.qchat_channel_member_delete_error);
                addLiveData.postValue(addResult);
            }
        });
    }
     ...
     */
    public void addMember(long serverId, long channelId, List<String> accIdList, QChatChannelModeEnum type) {
        QChatChannelRepo.addChannelBlackWhiteMembers(serverId, channelId, accIdList, type, new FetchCallback<Void>() {
