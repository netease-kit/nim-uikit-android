...
public class QChatRoleMemberActivity extends CommonActivity {
    ...
    public void initView() {
        ...
        binding.rlyMemberModify.setOnClickListener(v -> {
            Intent intent = new Intent(QChatRoleMemberActivity.this, QChatMemberSelectorActivity.class);
            intent.putExtra(QChatConstant.SERVER_ID, roleInfo.getServerId());
            intent.putExtra(QChatConstant.REQUEST_MEMBER_FILTER_KEY, QChatConstant.REQUEST_MEMBER_FILTER_ROLE);
    ...
    public void initData() {
        roleInfo = (QChatServerRoleInfo) getIntent().getSerializableExtra(QChatConstant.SERVER_ROLE_INFO);
        ...
        CommonMoreRecyclerViewDecorator<QChatServerRoleMemberInfo> decorator = new CommonMoreRecyclerViewDecorator<>(binding.rvMembers, layoutManager, memberAdapter);
        decorator.setLoadMoreListener(data -> getMembers(data == null ? 0 : data.getCreateTime(), data == null ? null : data.getAccId()));
    }
    ...
    private void showDeleteConfirmDialog(QChatServerRoleMemberInfo item) {
        ...
        dialog.setTitleStr(getResources().getString(R.string.qchat_delete_member))
                .setContentStr(String.format(getResources().getString(R.string.qchat_delete_some_member), nick))
    ...
    private void deleteMember(QChatServerRoleMemberInfo item) {
        ...
        QChatRoleRepo.removeServerRoleMember(roleInfo.getServerId(),
                roleInfo.getRoleId(), accIds, new QChatCallback<List<String>>(getApplicationContext()) {
                    @Override
    ...
    private void addMembers(List<String> accIds) {
        QChatRoleRepo.addServerRoleMember(roleInfo.getServerId(), roleInfo.getRoleId(), accIds,
    ...
    private void getMembers(long timeTag, String accId) {
        QChatRoleRepo.fetchServerRoleMember(roleInfo.getServerId(),
                roleInfo.getRoleId(), timeTag, PAGE_SIZE, accId,
