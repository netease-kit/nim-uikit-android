...
public class ChannelCreateViewModel extends BaseViewModel {
     ...
     * create new channel
     */
    public void createChannel(Long serverId, String name, String topic, QChatChannelModeEnum typeEnum) {
        ALog.d(TAG, "createChannel", "param:" + serverId + "," + name + "," + topic);
        QChatChannelRepo.createChannel(serverId, name, topic, typeEnum, QChatChannelTypeEnum.Message, new FetchCallback<QChatChannelInfo>() {
            @Override
            ...
            public void onException(@Nullable Throwable exception) {
                fetchResult.setError(QChatConstant.ERROR_CODE_CHANNEL_CREATE, R.string.qchat_channel_create_error);
                resultLiveData.postValue(fetchResult);
