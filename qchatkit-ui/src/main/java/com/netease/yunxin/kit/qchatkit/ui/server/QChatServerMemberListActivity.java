...
public class QChatServerMemberListActivity extends BaseActivity {
        ...
        launcher = registerForActivityResult(new ActivityResultContracts.StartActivityForResult(), result -> {
            ...
            if (data != null) {
                ...
                if (friends != null && !friends.isEmpty()) {
                    NetworkUtils.isConnectedToastAndRun(this, () -> QChatServerRepo.inviteServerMembers(serverId, friends, new FetchCallback<List<String>>() {
                        @Override
                        public void onSuccess(@Nullable List<String> failedList) {
                            handleInviteTipVisible(true);
                            loadMore(0, true);
                            if (failedList != null && !failedList.isEmpty()) {
                                Toast.makeText(QChatServerMemberListActivity.this, "failed list is " + failedList, Toast.LENGTH_SHORT).show();
                            }
                        }
                        ...
                        public void onFailed(int code) {
                            ...
                            ALog.w(TAG, msg);
                            Toast.makeText(QChatServerMemberListActivity.this, msg, Toast.LENGTH_SHORT).show();
                        }
                    ...
                    if (type == MEMBER_OPERATOR_TYPE_CHANGED) {
                        QChatServerRepo.fetchServerMemberInfoWithRolesByAccId(serverId, accId, new FetchCallback<QChatServerMemberWithRoleInfo>() {
                            @Override
    ...
    private void initView() {
        ...
        adapter.setItemClickListener((data, holder) -> {
            Intent intent = new Intent(QChatServerMemberListActivity.this, QChatServerMemberInfoActivity.class);
        ...
        LoadMoreRecyclerViewDecorator<QChatServerMemberWithRoleInfo> decorator = new LoadMoreRecyclerViewDecorator<>(binding.ryMemberList, layoutManager, adapter);
        decorator.setLoadMoreListener(data -> {
            NextInfo info;
    ...
    private void loadMore(long timeTag, boolean clear) {
        QChatServerRepo.fetchServerMemberInfoWithRolesList(serverId, timeTag, LOAD_MORE_LIMIT, new FetchCallback<List<QChatServerMemberWithRoleInfo>>() {
            @Override
    ...
    private void prepareAnim() {
        ...
        ObjectAnimator showTransYAnimator = ObjectAnimator.ofFloat(binding.cvInviteMemberTip,
                "translationY", -PxUtils.dpToPx(this, DEFAULT_DP_TIP_TRANSLATION_Y), 0f)
                .setDuration(500);
        ObjectAnimator dismissTransYAnimator = ObjectAnimator.ofFloat(binding.cvInviteMemberTip,
                "translationY", 0f, -PxUtils.dpToPx(this, DEFAULT_DP_TIP_TRANSLATION_Y))
                .setDuration(500);
