...
public class QChatServerFragment extends BaseFragment {
    ...
    /**
     * The observer of handling action with different types. Receiving the action to update server list.
     */
    private final Observer<Pair<Integer, Long>> notificationObserver = new Observer<Pair<Integer, Long>>() {
        @Override
        public void onChanged(Pair<Integer, Long> integerLongPair) {
            switch (integerLongPair.first) {
                case TYPE_REFRESH_CHANNEL:
                    fragment.refreshData(integerLongPair.second, ObserverUnreadInfoResultHelper.getUnreadInfoMap(integerLongPair.second));
                    break;
     ...
     */
    private final Observer<ResultInfo<QChatServerInfo>> observerForUpdate = new Observer<ResultInfo<QChatServerInfo>>() {
        @Override
        public void onChanged(ResultInfo<QChatServerInfo> qChatServerInfoResultInfo) {
            if (qChatServerInfoResultInfo.getSuccess() && qChatServerInfoResultInfo.getValue() != null) {
     ...
     */
    private final Observer<List<Long>> observerUnreadInfo = new Observer<List<Long>>() {
        @Override
    ...
    public void onViewCreated(@NonNull View view, Bundle savedInstanceState) {
        ...
        manager.beginTransaction()
                .add(R.id.ryChannelFragment, fragment)
                .commitAllowingStateLoss();
        // creating or join a server.
        viewBinding.ivAddServer.setOnClickListener(v -> startActivity(new Intent(getContext(), QChatCreateWayActivity.class),
                ActivityOptionsCompat.makeCustomAnimation(v.getContext(), R.anim.anim_from_bottom_to_top, R.anim.anim_empty_with_time).toBundle()));
        // render server list.
        adapter = new QChatFragmentServerAdapter(getContext());
        adapter.setItemClickListener((data, holder) -> fragment.updateData(data.serverInfo, data.unreadInfoItemMap));
        ...
        LoadMoreRecyclerViewDecorator<QChatFragmentServerInfo> loadMoreRecyclerViewDecorator = new LoadMoreRecyclerViewDecorator<>(viewBinding.ryServerList, layoutManager, adapter);
    ...
    private class ObserverWrapper implements Observer<ResultInfo<List<QChatServerInfo>>> {
        ...
        public void onChanged(ResultInfo<List<QChatServerInfo>> listResultInfo) {
            if (listResultInfo.getSuccess()) {
                ...
                Toast.makeText(getContext(), getString(R.string.qchat_server_request_fail) + (msg != null ? msg.getCode() : ""), Toast.LENGTH_SHORT).show();
            }
        }
    }
}
