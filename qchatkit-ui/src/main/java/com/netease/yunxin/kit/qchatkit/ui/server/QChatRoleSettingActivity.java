...
public class QChatRoleSettingActivity extends CommonActivity {
    ...
    public void initView() {
        ...
        binding.rlyMemberModify.setOnClickListener(v -> {
    ...
    private void registerResult() {
        memberLauncher = registerForActivityResult(new ActivityResultContracts.StartActivityForResult(), result -> {
            if (result.getResultCode() == RESULT_OK && result.getData() != null) {
                int memberSize = result.getData().getIntExtra(QChatConstant.REQUEST_MEMBER_SIZE_KEY, (int) role.getMemberCount());
    ...
    private void updateAuths(CompoundButton switchCompat, QChatRoleResourceEnum key) {
        ...
        QChatRoleRepo.updateRole(role.getServerId(), role.getRoleId(),
                null, null, null, auths,
    ...
    public void initData() {
        ...
        binding.scManagerServer.setChecked(role.getAuths().get(QChatRoleResourceEnum.MANAGE_SERVER)
                == QChatRoleOptionEnum.ALLOW);

        binding.scManageChannelPro.setChecked(role.getAuths().get(QChatRoleResourceEnum.MANAGE_CHANNEL)
                == QChatRoleOptionEnum.ALLOW);

        binding.scManagerRole.setChecked(role.getAuths().get(QChatRoleResourceEnum.MANAGE_ROLE)
                == QChatRoleOptionEnum.ALLOW);

        binding.scSendMessage.setChecked(role.getAuths().get(QChatRoleResourceEnum.SEND_MSG)
                == QChatRoleOptionEnum.ALLOW);

        binding.scModifySelfMember.setChecked(role.getAuths().get(QChatRoleResourceEnum.ACCOUNT_INFO_SELF)
                == QChatRoleOptionEnum.ALLOW);

        binding.scKickOut.setChecked(role.getAuths().get(QChatRoleResourceEnum.KICK_SERVER)
                == QChatRoleOptionEnum.ALLOW);

        binding.scModifyOtherMember.setChecked(role.getAuths().get(QChatRoleResourceEnum.ACCOUNT_INFO_OTHER)
                == QChatRoleOptionEnum.ALLOW);

        binding.scInviteMember.setChecked(role.getAuths().get(QChatRoleResourceEnum.INVITE_SERVER)
                == QChatRoleOptionEnum.ALLOW);

        binding.scManagerMember.setChecked(role.getAuths().get(QChatRoleResourceEnum.MANAGE_BLACK_WHITE_LIST)
                == QChatRoleOptionEnum.ALLOW);
    ...
    private void setListener() {
        binding.rlyManagerServer.setOnClickListener(v -> updateAuths(binding.scManagerServer, QChatRoleResourceEnum.MANAGE_SERVER));

        binding.rlyManageChannelPro.setOnClickListener(v -> updateAuths(binding.scManageChannelPro, QChatRoleResourceEnum.MANAGE_CHANNEL));

        binding.rlyManagerRole.setOnClickListener(v -> updateAuths(binding.scManagerRole, QChatRoleResourceEnum.MANAGE_ROLE));

        binding.rlySendMessage.setOnClickListener(v -> updateAuths(binding.scSendMessage, QChatRoleResourceEnum.SEND_MSG));

        binding.rlyModifySelfMember.setOnClickListener(v -> updateAuths(binding.scModifySelfMember, QChatRoleResourceEnum.ACCOUNT_INFO_SELF));

        binding.rlyKickOut.setOnClickListener(v -> updateAuths(binding.scKickOut, QChatRoleResourceEnum.KICK_SERVER));

        binding.rlyModifyOtherMember.setOnClickListener(v -> updateAuths(binding.scModifyOtherMember, QChatRoleResourceEnum.ACCOUNT_INFO_OTHER));

        binding.rlyInviteMember.setOnClickListener(v -> updateAuths(binding.scInviteMember, QChatRoleResourceEnum.INVITE_SERVER));

        binding.rlyManagerMember.setOnClickListener(v -> updateAuths(binding.scManagerMember, QChatRoleResourceEnum.MANAGE_BLACK_WHITE_LIST));
