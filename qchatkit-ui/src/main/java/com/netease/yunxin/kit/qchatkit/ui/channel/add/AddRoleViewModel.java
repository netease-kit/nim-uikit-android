...
public class AddRoleViewModel extends BaseViewModel {
    ...
    private void fetchRoleData(long serverId, long channelId, long offset) {
        QChatRoleRepo.fetchServerRolesWithoutChannel(serverId, channelId, offset, QChatConstant.MEMBER_PAGE_SIZE, new FetchCallback<List<QChatServerRoleInfo>>() {
            @Override
            public void onSuccess(@Nullable List<QChatServerRoleInfo> param) {
                ...
                if (param != null && param.size() > 0) {
                    for (int index = 0; index < param.size(); index++) {
                        ...
                        int bottomRadius = index == param.size() - 1 ? QChatConstant.CORNER_RADIUS_ARROW : 0;
                        QChatArrowBean bean = new QChatArrowBean(roleInfo.getName(), topRadius, bottomRadius);
                        bean.param = roleInfo;
                        addList.add(bean);
                    }
            ...
            public void onException(@Nullable Throwable exception) {
                ...
                fetchResult.setError(QChatConstant.ERROR_CODE_CHANNEL_ROLE_FETCH, R.string.qchat_fetch_role_error);
                roleLiveData.postValue(fetchResult);
            }
        });
    }
    ...
    public void addChannelRole(long channelId, QChatServerRoleInfo roleInfo) {
        QChatRoleRepo.addChannelRole(roleInfo.getServerId(), channelId, roleInfo.getRoleId(), new FetchCallback<QChatChannelRoleInfo>() {
            @Override
            ...
            public void onException(@Nullable Throwable exception) {
                String errorMsg = exception != null ? exception.getMessage() : "";
                ALog.d(TAG, "addMemberToChannel", "onException:" + errorMsg +","+ roleInfo.getRoleId());
                addResult.setError(QChatConstant.ERROR_CODE_CHANNEL_MEMBER_ADD, R.string.qchat_add_role_error);
                addLiveData.postValue(addResult);
            }
        });
    }
