...
/**
 * Watch image or video info view model
 * update image/video file downloading progress for watch image/video page
 */
public class WatchImageVideoViewModel extends BaseViewModel {
    ...
    private void onDownloadStart(IMMessage message) {
        ALog.i(TAG, "on download start -->> " + (((FileAttachment) message.getAttachment()).getPathForSave()));
    ...
    private void onDownloadSuccess(IMMessage message) {
        ALog.i(TAG, "on download success -->> " + (((FileAttachment) message.getAttachment()).getPath()));
    ...
    public void downloadAttachment(IMMessage message) {
        ChatMessageRepo.downloadAttachment(message, false, new FetchCallback<Void>() {
            @Override
