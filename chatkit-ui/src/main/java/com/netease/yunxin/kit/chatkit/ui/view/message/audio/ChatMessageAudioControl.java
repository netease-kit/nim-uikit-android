...
public class ChatMessageAudioControl extends BaseAudioControl<IMMessageInfo> {
    ...
    public void startPlayAudioDelay(
        ...
        if (!file.exists()) {
            ChatMessageRepo.downloadAttachment(message.getMessage(), false, new ChatCallback<Void>() {
                @Override
    ...
    private void startPlayAudio(
        ...
        if (StorageUtil.isExternalStorageExist()) {
            if (startAudio(new ChatMessageAudioPlayable(message), audioControlListener, audioStreamType, resetOrigAudioStreamType, delayMillis)) {
