...
public class AitContactSelectorDialog extends BottomSheetDialog {
    ...
    public AitContactSelectorDialog(@NonNull Context context, int themeResId) {
        ...
        binding = ChatMessageAitSelectorDialogBinding.inflate(LayoutInflater.from(getContext()), null, false);
        setContentView(binding.getRoot(), new FrameLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
                ScreenUtil.getDisplayHeight() * 2 / 3));
    ...
    private void initViews() {
        ...
        adapter.setOnItemSelectListener(item -> {
            if (listener != null) {
