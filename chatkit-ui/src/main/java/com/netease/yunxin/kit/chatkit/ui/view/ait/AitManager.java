        ...
        if (delete) {
            ...
            if (s.toString().equals("@") && !TextUtils.isEmpty(tid)) {
                ...
                dialog.setOnItemSelectListener(item -> {
                    if (item == null) {
                        // ait all
                        insertAitMemberInner(AitContactsModel.ACCOUNT_ALL,
                                mContext.getString(R.string.chat_team_ait_all), curPos, false);
                    } else {
                        NimUserInfo userInfo = item.getUserInfo();
                        if (userInfo != null) {
                            insertAitMemberInner(userInfo.getAccount(), item.getName(), curPos, false);
                        }
                    }
                });
                dialog.show();
