...
public class ChatAudioMessageViewHolder extends ChatBaseMessageViewHolder {
    ...
    private final ChatMessageAudioControl.AudioControlListener onPlayListener = new ChatMessageAudioControl.AudioControlListener() {
    ...
    public void bindData(ChatMessageBean message, ChatMessageBean lastMessage) {
        ...
        audioBinding.container.setOnClickListener(v -> {
            initPlayAnim();
            audioControl.setEarPhoneModeEnable(ConfigRepo.INSTANCE.getAudioPlayModel() == AUDIO_PLAY_EARPIECE);
            audioControl.startPlayAudioDelay(CLICK_TO_PLAY_AUDIO_DELAY, message.getMessageData(), onPlayListener);
        });
    }

    private void setAudioLayout(ChatMessageBean message) {
        AudioAttachment audioAttachment = (AudioAttachment) message.getMessageData().getMessage().getAttachment();
        ...
        RelativeLayout.LayoutParams timeLp = (RelativeLayout.LayoutParams) audioBinding.tvTime.getLayoutParams();
    ...
    protected boolean isMessagePlaying(ChatMessageBean message) {
        return audioControl.getPlayingAudio() != null
                && audioControl.getPlayingAudio().getMessage().isTheSame(message.getMessageData().getMessage());
    }
