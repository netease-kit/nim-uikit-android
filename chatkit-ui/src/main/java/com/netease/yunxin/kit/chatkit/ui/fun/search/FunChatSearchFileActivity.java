...
import com.netease.yunxin.kit.chatkit.ui.databinding.FunChatSearchFileListActivityBinding;
...
public class FunChatSearchFileActivity extends BaseLocalActivity {
  private static final String TAG = "FunChatSearchFileActivity";
  private FunChatSearchFileListActivityBinding binding;
  private FunChatSearchFileAdapter groupAdapter;
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    binding = FunChatSearchFileListActivityBinding.inflate(getLayoutInflater());
  ...
  private void initViewModel() {
    ...
    // 搜索文件消息结果回调
    searchFileViewModel.getSearchFileLiveData().observeForever(this::onLoadSearchResult);
   ...
   * @param result
   */
  private void onLoadSearchResult(FetchResult<List<V2NIMMessage>> result) {
    hideLoading();
    if (result.getData() != null && result.getLoadStatus() == LoadStatus.Success) {
      List<V2NIMMessage> messages = result.getData();
      MessageSearchUtils.groupMessageByMonth(FunChatSearchFileActivity.this, fileGroups, messages);
      ...
      if (result.getLoadStatus() == LoadStatus.Error && result.getError() != null) {
        ErrorUtils.showErrorCodeToast(FunChatSearchFileActivity.this, result.getError().getCode());
      }
  ...
  private void initRecyclerView() {
    ...
    groupAdapter = new FunChatSearchFileAdapter(this, fileGroups);
    groupAdapter.setItemClickListener(
        new IItemClickListener<V2NIMMessage>() {
          ...
          public boolean onMessageClick(View view, int position, V2NIMMessage message) {
            ChatUtils.openFile(FunChatSearchFileActivity.this, message);
            return true;
          }
  ...
  public void showBottomActionPop(V2NIMMessage message) {
    ...
    actions.add(
        new ActionItem(
            ...
            .setTitleColorResId(R.color.color_333333));
    BottomChoiceDialog dialog = new BottomChoiceDialog(FunChatSearchFileActivity.this, actions);
          ...
          public void onChoice(@NonNull String type) {
            if (ActionConstants.POP_ACTION_TRANSMIT.equals(type)) {
              ...
              ChatUtils.startForwardSelector(
                  FunChatSearchFileActivity.this,
                  RouterConstant.PATH_FUN_FORWARD_SELECTOR_PAGE,
                  false,
                  forwardLauncher);
