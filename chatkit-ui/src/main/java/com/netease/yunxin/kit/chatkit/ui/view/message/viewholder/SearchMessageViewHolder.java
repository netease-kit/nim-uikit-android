...
public class SearchMessageViewHolder extends BaseViewHolder<ChatSearchBean> {
    ...
    public void onBindData(ChatSearchBean data, int position) {
        if (data != null) {
            viewBinding.cavIcon.setData(data.getAvatar(),
                    data.getNickName(), AvatarColor.avatarColor(data.getAccount()));

            viewBinding.tvNickName.setText(data.getNickName());

            viewBinding.tvMessage.setText(data.getSpannableString(viewBinding.getRoot().getContext().getResources().getColor(com.netease.yunxin.kit.common.ui.R.color.color_337eff)));

            viewBinding.tvTime.setText(TimeFormatUtils.formatMillisecond(viewBinding.getRoot().getContext(), data.getTime()));
