...
public class TeamMemberListAdapter extends TeamCommonAdapter<UserInfoWithTeam, TeamMemberListItemBinding> {
    ...
    public TeamMemberListAdapter(Context context, TeamTypeEnum teamTypeEnum, Class<TeamMemberListItemBinding> viewBinding) {
        super(context, viewBinding);
        this.teamTypeEnum = teamTypeEnum;
    }

    @Override
    public void onBindViewHolder(TeamMemberListItemBinding binding, int position, UserInfoWithTeam data, int bingingAdapterPosition) {
        binding.tvUserName.setText(data.getName());
        if (data.getTeamInfo().getType() == TeamMemberType.Owner && teamTypeEnum == TeamTypeEnum.Advanced) {
    ...
    public void filter(CharSequence sequence) {
        ...
        List<UserInfoWithTeam> filterResult = FilterUtils.filter(backupTotalData,
                userInfoWithTeam -> {
                    ...
                    boolean accIdContains = userInfoWithTeam.getTeamInfo().getAccount().contains(sequence);
                    if (accIdContains) {
                        userInfoWithTeam.setSearchPoint(100 + userInfoWithTeam.getTeamInfo().getAccount().length());
                        return true;
                    }
                    return false;
                });
        Collections.sort(filterResult, (o1, o2) -> {
            if (o1 == o2) {
                return 0;
            }
            if (o1 == null) {
