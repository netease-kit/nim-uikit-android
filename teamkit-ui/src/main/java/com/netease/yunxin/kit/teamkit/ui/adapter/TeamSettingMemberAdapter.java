...
public class TeamSettingMemberAdapter extends TeamCommonAdapter<UserInfoWithTeam, TeamSettingUserItemBinding> {
    ...
    @Override
    public void onBindViewHolder(TeamSettingUserItemBinding binding, int position, UserInfoWithTeam data, int bingingAdapterPosition) {
        NimUserInfo userInfo = data.getUserInfo();
        if (userInfo != null) {
            binding.cavUserIcon.setData(userInfo.getAvatar(), data.getName(), ColorUtils.avatarColor(data.getUserInfo().getAccount()));
            binding.cavUserIcon.setOnClickListener(v -> {
